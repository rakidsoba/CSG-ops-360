...
router.post('/submit', requirePermissions('deployments:read'), async (req, res) => {
  ...
  try {
    ...
    for (const rec of records) {
      ...
      const { rows } = await client.query(
        `INSERT INTO attendance_records
          (deployment_id, guard_id, site_id, shift_id, attendance_date, status, notes,
           captured_at, synced_at, submitted_by, client_event_id, latitude, longitude, gps_accuracy_m)
      ...
      results.push({ client_event_id: rec.client_event_id, status: 'accepted', attendance_id: rows[0].id });
    }
